...
pub async fn progress() {
    ...
    loop {
        ...
        if is_key_down(KeyCode::Up) {
            for (collider1, collider2, intersecting) in narrow_phase.intersections_with(floor_sensor_collider_handle) {
                if intersecting {
        ...
        while let Ok(intersection_event) = intersection_recv.try_recv() {
            ...
            if intersection_event.collider1 == floor_sensor_collider_handle ||
                intersection_event.collider2 == floor_sensor_collider_handle {
...
impl DynamicRectangle {
    ...
    fn draw(&self, rigid_body_set: &RigidBodySet) {
        ...
        let width = METERS_PLAYER_HEIGHT - METERS_SENSOR_MARGIN * 2.0;
        let height = METERS_PLAYER_HEIGHT/4.0 + METERS_SENSOR_MARGIN;
        draw_rectangle(x-width / 2.0, y-height/2.0-METERS_SENSOR_MARGIN / 2.0 + METERS_PLAYER_HEIGHT/8.0 - METERS_PLAYER_HEIGHT/2.0,
                       width, height, PURPLE);
    }
    fn getPosition(&self, rigid_body_set: &RigidBodySet) -> Vec2 {
        let body = &rigid_body_set[self.handle];
